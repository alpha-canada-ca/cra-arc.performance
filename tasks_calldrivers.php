...
<?php
   ...
   //Query strings fot Task id

    // $taskId = $_GET['taskId'] ?? "recXkhR8zOWnR83TU"; //rec8UrXPrTCryLYF2
    // //$taskId = $_GET['taskId'] ?? "rec8UrXPrTCryLYF2"; //
    // $dr = $_GET['dr'] ?? "week";
    // $lang = $_GET['lang'] ?? "en";
?>
    ...
    $taskId = $_GET['taskId'] ?? "recUGePC2eHIoyKYQ"; //recUGePC2eHIoyKYQ; //rec5ZjiYAlY8pjtnl;
    //$taskId = $_GET['taskId'] ?? "recmoIEMloY2HUjYN"; //rec8UrXPrTCryLYF2
    ...
    <!-- <style>
    h2.h3 span {color: #345ea5; text-transform:uppercase; font-variant: all-small-caps; letter-spacing: 5px; font-size: 16px; display: flex; padding-bottom: 5px;}
    </style> -->
    ...
    ?>
      ...
      <?php
              ...
              if (count($weeklyMonthRanges) > 1) {
                  ...
                  $params1 = array("filterByFormula" => "AND(IS_AFTER({CALL_DATE}, '$wMrS1'), IS_BEFORE({CALL_DATE}, DATEADD('$wMrE1',1,'days')), (Lookup_Task = '$taskName'))"); // array( "filterByFormula" => 'SEARCH(Task, "'.implode($taskArray, ',').'") != ""');
                  $params2 = array("filterByFormula" => "AND(IS_AFTER({CALL_DATE}, '$wMrE1'), IS_BEFORE({CALL_DATE}, DATEADD('$wMrE2',1,'days')), (Lookup_Task = '$taskName'))"); // array( "filterByFormula" => 'SEARCH(Task, "'.implode($taskArray, ',').'") != ""');
            ...
            //--------------------------------------------------------------
              ...
              //if (count($weeklyMonthRanges) > 1) {
                ...
                // $currentQuarter = ceil($month / 3);
                  ...
                  //$params = array("filterByFormula" => "AND(IS_AFTER({CALL_DATE}, '$s'), IS_BEFORE({CALL_DATE}, DATEADD('$e1',1,'days')))");Lookup_Task
          ...
          ?>
              <div class="row mb-4">
                <div class="col-lg-12 col-md-12">
                  <p>No Call drivers data associated with this Task for selected date Range.</p>
                  <!-- <div class="card">
                    <div class="card-body pt-2">
